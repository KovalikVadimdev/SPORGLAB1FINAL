    ...
    switch (message) {
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDCANCEL: // Обробка натискання кнопки "Закрити"
...
INT_PTR CALLBACK CustomMessageBoxProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDC_BUTTON1:
...
INT_PTR CALLBACK SecondDialogProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) {
    switch (message) {
        case WM_COMMAND:
            switch (LOWORD(wParam)) {
                case IDOK:
                    ...
                    return (INT_PTR) TRUE;
                case IDCANCEL:
                    MessageBox(hDlg, "Dialog will be closed", "Close Dialog", MB_OK);
                    EndDialog(hDlg, IDC_BUTTON2);
                    return (INT_PTR) TRUE;
            }
            break;
...
void CreateOldMenu(HWND hWnd) {
    ...
    AppendMenu(hAboutMenu, MF_STRING, ID_MENU_ABOUTTHEPROGRAM, "About the program");
    AppendMenu(hAboutMenu, MF_STRING, ID_MENU_SECONDDIALOGBOX, "Second dialog box");
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        ...
        case WM_COMMAND:                //Повідомлення від меню
            switch (LOWORD(wParam)) {
                case ID_MENU_ABOUTTHEPROGRAM:
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_DIALOG1), hWnd, AboutDialogProc);
                    break;
                case ID_MENU_SECONDDIALOGBOX:
                    DialogBox(hInst, MAKEINTRESOURCE(IDD_DIALOG2), hWnd, SecondDialogProc);
                    break;
                case IDS_STRING101:
        ...
        case WM_LBUTTONDBLCLK:                //Подвійний щелчок лівою кнопкою миші
            MessageBox(hWnd, "Double-click the left mouse button", "Double-click", MB_OK);
            break;
        ...
        case WM_CLOSE: {                //Закрити вікно
            HWND hDlg = CreateDialog(hInst, MAKEINTRESOURCE(IDD_DIALOGBAR), hWnd, CustomMessageBoxProc);
